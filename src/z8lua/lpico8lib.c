...
static int pico8_sqrt(lua_State* l) {
	...
	/* Fast path: integer input (no fractional bits).
	 * Uses a 32-bit-only restoring square root, avoiding 64-bit arithmetic
	 * entirely.  This covers the dominant PICO-8 pattern of computing
	 * sqrt(x*x + y*y) with integer screen coordinates, and is universally
	 * applicable to any cart that calls sqrt() with a whole number.
	 *
	 * On ARM9 (N-Gage) every 64-bit op is emulated in software (~3-4 native
	 * instructions each).  Replacing the 64-bit loop with two 32-bit loops
	 * (8 integer bit-pairs + 16 fractional bit-pairs) gives ~3-4x fewer
	 * cycles for the common case with no lookup table and zero extra RAM.
	 *
	 * The restoring algorithm computes the same floor(sqrt(n)*2^16) value
	 * as the 64-bit path - both are exact integer operations on the same
	 * mathematical input - so results are bit-identical to PICO-8. */
	if ((v & 0xFFFF) == 0) {
		uint32_t n = (uint32_t)v >> 16;  /* integer part, in [1, 32767] */
		uint32_t root = 0;
		uint32_t rem = 0;
		uint32_t t;
		int i;

		/* Integer phase: 8 bit-pairs cover all of n.
		 * fix32_t positive integers are at most 32767 (15 bits), so bits
		 * 31..16 of n are always zero and the loop starts at bit-pair 7. */
		for (i = 7; i >= 0; --i) {
			rem = (rem << 2) | ((n >> (i * 2)) & 3u);
			t = (root << 2) | 1u;
		...
		/* Fractional phase: 16 more bit-pairs produce the fix32_t fraction. */
		for (i = 0; i < 16; ++i) {
			rem <<= 2;
			t = (root << 2) | 1u;
			if (rem >= t) { rem -= t; root = (root << 1) | 1u; }
		...
		lua_pushnumber(l, (fix32_t)root);
		return 1;
	}

	/* General path: fractional fix32_t input - keep the 64-bit algorithm. */
	{
		int64_t root = 0, x = ((int64_t)v) << 16;
		int64_t a;
